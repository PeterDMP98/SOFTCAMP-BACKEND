...
export const registroReproduccionCreateSchema = z.object({
  id_madre: z.number().int().positive("ID de madre inválido"),
  id_padre: z.number().int().positive("ID de padre inválido").optional().nullable(),
...
export const registroReproduccionUpdateSchema = z.object({
  ...
  id_hijo: z.number().int().positive("ID de hijo inválido").optional().nullable(),
  detalles: z.string().max(500).optional().nullable()
});
